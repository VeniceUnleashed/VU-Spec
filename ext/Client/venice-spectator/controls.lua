...
function SpectatorControls:__init()
	self.m_RotateX = 0.0
	self.m_RotateY = 0.0
	self.m_MoveX = 0.0
	self.m_MoveY = 0.0
...
function SpectatorControls:RotateX(p_Transform, p_Vector)
	local x = Vec3(
...
function SpectatorControls:OnUpdateInput(p_Delta)
    if SpectatorManager:GetCameraMode() == SpectatorCameraMode.Disabled then
        return
    end

	-- Update the controls.
	self:UpdateSpectatorControls(p_Delta)
...
function SpectatorControls:UpdateCameraControls(p_Delta)
...
function SpectatorControls:UpdateSpectatorControls(p_Delta)
	if InputManager:WentKeyDown(InputDeviceKeys.IDK_Space) then
		if SpectatorManager:GetCameraMode() == SpectatorCameraMode.FirstPerson then 
			SpectatorManager:SetCameraMode(SpectatorCameraMode.ThirdPerson)
		elseif SpectatorManager:GetCameraMode() == SpectatorCameraMode.ThirdPerson then 
			SpectatorManager:SetCameraMode(SpectatorCameraMode.FreeCamera)
		else
			SpectatorManager:SetCameraMode(SpectatorCameraMode.FirstPerson)
		end
	end
end

function SpectatorControls:UpdateFreeCamera(p_Delta)
	local s_RotateX = -(self.m_RotateX * self.m_SimTickCount)
	local s_RotateY = -(self.m_RotateY * self.m_SimTickCount)

	local s_Transform = SpectatorManager:GetFreecameraTransform()

	-- Check for NaN
	if s_Transform.trans.x ~= s_Transform.trans.x or
	   s_Transform.up.x ~= s_Transform.up.x or
	   s_Transform.forward.x ~= s_Transform.forward.x or
	   s_Transform.left.x ~= s_Transform.left.x then
...
function SpectatorControls:UpdateThirdPerson(p_Delta)
